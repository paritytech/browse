...
function secs(from) {
  return ((performance.now() - from) / 1000).toFixed(1)
}
...
async function main() {
  ...
  const paraSpec = readFileSync(
    new URL('./chain-specs/asset-hub-paseo.json', import.meta.url),
    'utf-8'
  )
  ...
  log(
    `Step 2 done: ${allLabels.size} labels from ${storesWithData} stores (${secs(t2)}s, first at ${firstLabelAt}s)`
  )
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    const name = metaResults[i * 3 + 1]?.success
      ? decodeString(metaResults[i * 3 + 1].returnData)
      : ''
    const desc = metaResults[i * 3 + 2]?.success
      ? decodeString(metaResults[i * 3 + 2].returnData)
      : ''
    const live = ch !== '0x'
  ...
  log(
    `Step 3: metadata fills in within ${secs(t3)}s → update cards with description + live status.`
  )
  log(
    `User sees first real content at sync + 1s + ${firstLabelAt}s = ~${(parseFloat(secs(syncStart)) + 1 + parseFloat(firstLabelAt || '0')).toFixed(0)}s total.`
  )
