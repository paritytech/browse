...
function decodeStringArray(hex) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (strLen === 0) { strs.push(""); continue; }
...
async function reviveCall(api, contractAddress, encodedData) {
  ...
  const flagsStr = typeof ok.flags === "object" && ok.flags?.toString ? ok.flags.toString() : String(ok.flags ?? 0);
...
async function main() {
  ...
  const storesRaw = await reviveCall(api, CONTRACTS.STORE_FACTORY, encodeNoArgs(SEL.getAllDeployedStores));
  ...
  for (const label of uniqueLabels) {
    ...
    try {
      const chRaw = await reviveCall(api, CONTRACTS.CONTENT_RESOLVER, encodeBytes32(SEL.contenthash, node));
    ...
    try {
      const descRaw = await reviveCall(api, CONTRACTS.CONTENT_RESOLVER, encodeText(node, "description"));
