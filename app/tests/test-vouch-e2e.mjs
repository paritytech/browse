...
async function executeWrite(pjsApi, account, contractAddress, callData) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    const result = await new Promise((resolve) => {
      const timeout = setTimeout(() => resolve({ success: false, error: 'timeout' }), 120000)

      tx.signAndSend(account, { nonce: -1, tip: 10_000_000n }, (result) => {
        if (result.status.isInBlock || result.status.isFinalized) {
          clearTimeout(timeout)
          ...
          for (const { event } of result.events) {
            if (event.section === 'system' && event.method === 'ExtrinsicFailed') {
              ...
              if (error.isModule) {
                try {
                  const decoded = pjsApi.registry.findMetaError(error.asModule)
...
async function pjsReviveCall(pjsApi, origin, contractAddress, callData) {
  ...
  try {
    ...
    const execResult = json.result?.ok || json.result?.Ok || (json.data !== undefined ? json : null)
    if (execResult) {
      const flags =
        typeof execResult.flags === 'number' ? execResult.flags : (execResult.flags?.bits ?? 0)
...
async function testRegistryReads(papiApi) {
  ...
  try {
    const fakeSubject = '0x' + 'aa'.repeat(20)
    const raw = await papiReviveCall(papiApi, REGISTRY, encodeList(fakeSubject, 0, 10))
...
async function testMulticallBatch(papiApi) {
  ...
  try {
    ...
    const raw = await papiReviveCall(papiApi, CONTRACTS.MULTICALL3, encodeAggregate3(calls))
    ...
    if (results.length === labels.length) {
      ...
      fail('multicall count()', `expected ${labels.length} results, got ${results.length}`)
    }
  } catch (e) {
...
async function testDotnsDiscovery(papiApi) {
  ...
  try {
    const raw = await papiReviveCall(papiApi, stores[0], `0x${SEL.getValues}`)
    const labels = decodeStringArray(raw)
    if (labels.length > 0) {
      pass(
        `getValues(store[0]) → ${labels.length} labels: [${labels.slice(0, 5).join(', ')}${labels.length > 5 ? '...' : ''}]`
      )
    } else {
  ...
  try {
    for (const store of stores) {
      try {
        const raw = await papiReviveCall(papiApi, store, `0x${SEL.getValues}`)
        const labels = decodeStringArray(raw)
    ...
    if (labelSet.size > 0) {
      pass(
        `full scan → ${labelSet.size} unique labels from ${scannedStores}/${stores.length} stores`
      )
    } else {
...
async function testVouchWritePath(papiApi, pjsApi, account, evmAddress) {
  ...
  try {
    const raw = await readCall(encodeGet(testSubject, SCHEMA_RATING, evmAddress))
  ...
  try {
    const raw = await readCall(encodeIsValid(testSubject, SCHEMA_RATING, evmAddress))
    const valid = decodeBool(raw)
    if (valid) {
  ...
  try {
    ...
    if (countOverwrite === countBefore + 1) {
      ...
      fail('count after overwrite', `expected ${countBefore + 1}, got ${countOverwrite}`)
    }
  } catch (e) {
  ...
  try {
    ...
    if (returnedValue === newRating) {
      ...
      fail('get() after overwrite value', `expected ${newRating}, got ${returnedValue}`)
    }
  } catch (e) {
  ...
  try {
    ...
    if (countRevoke === countBefore + 1) {
      ...
      fail('count after revoke', `expected ${countBefore + 1}, got ${countRevoke}`)
    }
  } catch (e) {
...
async function testAbiEncoding(papiApi) {
  ...
  try {
    ...
    if (bytes.slice(0, 2) === '01' && bytes.slice(2, 4) === '05' && bytes.slice(4, 6) === '00') {
  ...
  try {
    const computed = toHex(keccak_256(new TextEncoder().encode('discovery.rating.v1')))
