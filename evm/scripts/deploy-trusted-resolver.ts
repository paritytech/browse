...
async function main() {
  ...
  const TRUSTED_ATTESTER = requireEnv(
    "TRUSTED_ATTESTER",
    "The EVM address of the app allowed to issue certifications."
  ) as `0x${string}`;
  console.log(`AttestationService: ${ATTESTATION_SERVICE}`);
  console.log(`TrustedAttester: ${TRUSTED_ATTESTER}`);
  ...
  try {
    ...
    const constructorArgs = encodeAbiParameters(
      ...
      [ATTESTATION_SERVICE, TRUSTED_ATTESTER]
    );
