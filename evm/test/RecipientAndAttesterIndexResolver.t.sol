...
contract RecipientAndAttesterIndexResolverTest is Test {
    ...
    function test_onAttest_admitsBoundAndIndexes() public {
        vm.prank(service);
        ...
        assertTrue(resolver.identityHasAttested(app, SCHEMA, _identity(alicePk)));
    }
    ...
    function test_onAttest_sybilRejectsSameIdentityViaSecondProductAccount() public {
    ...
    function test_onRevoke_releasesIdentityAndReattestSucceeds() public {
        vm.startPrank(service);
        resolver.onAttest(_att(1, productA, app));
        resolver.onRevoke(_att(1, productA, app));

        assertFalse(resolver.identityHasAttested(app, SCHEMA, _identity(alicePk)));
        assertEq(resolver.countByRecipientAndSchema(app, SCHEMA), 0);
