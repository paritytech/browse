...
contract PublisherTest is Test {
    ...
    function test_version_returnsExpectedSemver() public view {
        assertEq(publisher.version(), "2.0.0");
    }
    ...
    function test_publish_liteAllowsThreeWithinWindow() public {
        _mockStatus(alice, 1);
        ...
        assertEq(publisher.publishedCount(), 3);
    }

    function test_publish_liteFourthInWindowReverts() public {
        _mockStatus(alice, 1);
    ...
    function test_publish_liteSlotFreesAfterWindow() public {
        ...
        vm.warp(uint64(block.timestamp) + RATE_WINDOW);

        _publishAs(alice, "d");
        assertEq(publisher.publishedCount(), 4);
    }
    ...
    function test_publish_liteRepublishConsumesSlot() public {
        ...
        _publishAs(alice, "a");
        _publishAs(alice, "a"); // same label, state no-op but counts against cap
        _publishAs(alice, "a");

        // 4th call (any label) must be rate-limited.
        _mockOwner(_tokenIdOf("b"), alice);
        vm.expectRevert();
        vm.prank(alice);
        publisher.publish("b");

        // Only one entry was recorded despite three calls.
    ...
    function test_publish_tierUpgradeMidWindowGrantsHigherCap() public {
        ...
        _mockStatus(alice, 2);
        // Full now allows 2 more within the rolling window (5 total).
        _publishAs(alice, "d");
        _publishAs(alice, "e");
    ...
    function test_publish_tierDowngradeMidWindowAppliesLowerCap() public {
        ...
        _mockStatus(alice, 1);
        // Lite cap (3) is already met. Next call must revert.
        _mockOwner(_tokenIdOf("d"), alice);
        vm.expectRevert();
        vm.prank(alice);
        publisher.publish("d");
    ...
    function test_unpublish_doesNotConsumeRateSlot() public {
        _mockStatus(alice, 1);
        _publishAs(alice, "a");
        _publishAs(alice, "b");

        // Unpublish should not count against the rolling window.
        _mockOwner(_tokenIdOf("b"), alice);
        vm.prank(alice);
        publisher.unpublish("b");

        // One more publish should succeed (3rd slot still free), then 4th must revert
        // because the slot from "b" is still active in the rate-limit ring (unpublish
        // does not free rate slots).
        _publishAs(alice, "c");
        _mockOwner(_tokenIdOf("d"), alice);
        vm.expectRevert();
        vm.prank(alice);
