...
- **Versioned contracts.** Publisher and the attestation resolver/schema are arrays in `packages/browse-sdk/src/config.ts`. Reads union all versions, writes use index 0, and a redeploy prepends so index 0 is the newest. `PUBLISHER` deviates from that while a redeploy still has empty storage: it sits after the registry holding the listings and moves to index 0 once that state is migrated. A redeploy adds an entry, it does not replace one.
