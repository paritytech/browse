...
- **Genesis var is `NETWORK_GENESIS_HASH`.** There is no `VITE_ACTIVE_GENESIS`. Vite exposes it via `envPrefix: ['APP_','NETWORK_']` from the root `.env`. Code fallback is Paseo, the committed `.env` pins previewnet, and `.env.example` is Paseo, so mind the mismatch. Only two genesis values are known (paseo, previewnet). `dev:*` and `test:e2e` set it inline.
...
- **Versioned contracts.** Publisher and the attestation resolver/schema are arrays in `packages/browse-sdk/src/config.ts`. Reads union all versions and writes use index 0, but the two arrays reach index 0 differently. A resolver redeploy prepends, so index 0 is the newest. `PUBLISHER` is ordered write-target first: a proof-gated redeploy starts with empty storage, so it goes after the registry that still holds the listings and only moves to index 0 once that state is migrated. Either way a redeploy adds an entry, it does not replace one.
